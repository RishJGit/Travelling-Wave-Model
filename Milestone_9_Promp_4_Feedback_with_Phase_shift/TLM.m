...
for i = 2:Nt                    % 2 to 1000 in steps of 1
    ...
    delay_steps = round(500e-12/dt);
    phase_shift = pi/6;

    if i > delay_steps
        if abs(OutputR(i - delay_steps)) > 1e-3
            FeedbackR = OutputR(i - delay_steps);
            InjectedFeedback(i) = FeedbackR * exp(1i * phase_shift);
            InputR(i) = InjectedFeedback(i);
        else
            InjectedFeedback(i) = 0;
            InputR(i) = 0;
        end
    else
        InjectedFeedback(i) = 0;
    ...
    delay_steps = round(1000e-12/dt);
    phase_shift = pi/6;

    if i == delay_steps + 1
        FeedbackR = OutputR(i - delay_steps);                    % Right-side output
        InjectedFeedback(i) = FeedbackR * exp(1i * phase_shift); % Phase shift
        InputR(i) = InjectedFeedback(i);                         % Inject to right side
    end
    ...
    fftFeedback = fftshift(fft(InjectedFeedback));
    ...
    if mod(i,2000) == 0       % Only executed when i is multiple of plotN
        ...
        subplot(3,2,5)
        plot(time * 1e12, real(InjectedFeedback), 'k--'); hold on
        ...
        xlabel('time(ps)')
        ylabel('E (V/um)')
        legend('Injected Feedback (Right)', 'OutputR', 'OutputL', 'Location', 'east')
        hold off
        ...
        plot(omega, 20*log10(abs(fftOutput2)), 'r');
        plot(omega, 20*log10(abs(fftFeedback)), 'k--');  % From InjectedFeedback
        xlabel('Frequency (Hz)')
        ylabel('20 log_{10} |E|')
        legend('OutputR', 'OutputL', 'Feedback')
        xlim([-0.05e14 0.05e14])
        hold off


        subplot(3,2,6)
        phase1 = unwrap(angle(fftOutput1));
        phase2 = unwrap(angle(fftFeedback));
        plot(omega, phase1); hold on
        plot(omega, phase2);
        xlabel('Frequency (Hz)')
        ylabel('phase (E)')
        legend('OutputR', 'Feedback');
        hold off

        pause(0.01)
        % UNCOMMENT IF YOU WANT TO REPLICATE THE PLOTS I HAVE IN MY REPORT
%         figure(2)
%         clf
%         plot(omega, 20*log10(abs(fftOutput1)), 'b'); hold on
%         plot(omega, 20*log10(abs(fftOutput2)), 'r');
...
%         legend('OutputR', 'OutputL', 'Feedback')
%         xlim([-0.05e14 0.05e14])
%         title('Spectral Response with Feedback at 500 ps')
%         hold off
%         drawnow
% 
%         figure(3)
...
%         ylabel('E (V/\mum)')
%         legend('Injected Feedback (Right)', 'OutputR', 'OutputL', 'Location', 'east')
%         title('Field vs Time with Injected Feedback at 500 ps')
%         hold off
%         drawnow
