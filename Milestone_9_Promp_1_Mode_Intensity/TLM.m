...
figure('name', 'Fields')
If = nan(1, Nt);  % Forward mode intensity over time
Ir = nan(1, Nt);  % Backward mode intensity over time
...
% Define time range for plotting mode intensities
t_start = 0.000001e-12;  % 100 ps
t_end = Nt;    % 300 ps
...
for i = 2:Nt                    % 2 to 1000 in steps of 1
    ...
    % --- Mode intensities as a function of time ---
    If(i) = trapz(z, abs(Ef).^2);   % Forward mode intensity
    Ir(i) = trapz(z, abs(Er).^2);   % Backward mode intensity
    ...
    range_idx = find(time >= t_start & time <= t_end);
    ...
    if mod(i,2000) == 0       % Only executed when i is multiple of plotN
        ...
        figure('name', 'Mode Intensities Over Time')
        plot(time(range_idx) * 1e12, If(range_idx), 'r', 'DisplayName', '|E_f|^2'); hold on
        plot(time(range_idx) * 1e12, Ir(range_idx), 'b', 'DisplayName', '|E_r|^2');
        xlabel('Time (ps)')
        ylabel('Mode Intensity')
        legend
        title('Mode Intensities vs time')
        grid on
...
%% UNCOMMENT THIS PART IF YOU WISH TO SEE PLOTS SIMILAR TO THE ONES IN MY FINAL REPORT
% figure('Name', 'FFT plots at different time samples');
% hold on;
% 
% % Time points in % of total simulation
% pcts = [0.001, 0.25, 0.5, 0.75, 1, ...
%         1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5, ...
%         10, 15, 20, 25, 30, ...
%         50, 70, 90, 100];
% 
% colors = lines(length(pcts));  
% startIdx = round(0.001 * Nt);  
% 
% for k = 1:length(pcts)
%     endIdx = round((pcts(k)/100) * Nt);
...
%     end
% 
%     fftData = fftshift(abs(fft(OutputR(idxRange))));
%     omega = fftshift(wspace(time(idxRange)));
% 
%     plot(omega, 20*log10(fftData), 'Color', colors(k,:), ...
%         'DisplayName', sprintf('%.3f%%–%.1f%%', 0.001, pcts(k)));
% end
% 
% xlabel('Frequency (Hz)');
% ylabel('20 log_{10} |E|');
% title('FFT Snapshots: Growth of Modes Over Time');
% legend('Location', 'bestoutside');
% grid on;
% xlim([-0.03e14, 0.03e14]);
% 
% 
% disp('Simulation finished. Running FFT analysis...');
% 
% transient_range = round(0.0001*Nt):round(0.003*Nt);
% steady_range = round(0.0001*Nt):round(0.05*Nt);
% stable_range = round(0.0001*Nt):round(0.59*Nt);
% full_range = 1:Nt;
% 
% output_transient = OutputR(transient_range);
% output_steady = OutputR(steady_range);
% output_stable = OutputR(stable_range);
% output_full = OutputR(full_range);
% 
% time_transient = time(transient_range);
% time_steady = time(steady_range);
% time_stable = time(stable_range);
% time_full = time(full_range);
% 
% omega_transient = fftshift(wspace(time_transient));
% omega_steady = fftshift(wspace(time_steady));
% omega_stable = fftshift(wspace(time_stable));
% omega_full = fftshift(wspace(time_full));
% 
% fft_transient = fftshift(fft(output_transient));
% fft_steady = fftshift(fft(output_steady));
% fft_stable = fftshift(fft(output_stable));
% fft_full = fftshift(fft(output_full));
% 
% 
% 
% figure('Name', 'FFT Comparison (All Phases)', 'Color', 'w');
% 
% % Transient FFT
% subplot(2,2,1);
...
% xlabel('Frequency (Hz)');
% ylabel('20 log_{10} |E|');
% title('Modes approaching Steady-State');
% grid on;
% xlim([-0.02e14 0.02e14]);
% 
% % Stable FFT
% subplot(2,2,3);
% plot(omega_stable, 20*log10(abs(fft_stable)), 'g');
% xlabel('Frequency (Hz)');
% ylabel('20 log_{10} |E|');
% title('Modes Stable In Steady-State');
% grid on;
% xlim([-0.02e14 0.02e14]);
% 
% % Full FFT
% subplot(2,2,4);
% plot(omega_full, 20*log10(abs(fft_full)), 'k');
% xlabel('Frequency (Hz)');
% ylabel('20 log_{10} |E|');
% title('Modes over Full Simulation');
% grid on;
% xlim([-0.1e14 0.1e14]);
